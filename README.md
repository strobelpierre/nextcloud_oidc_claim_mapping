...
[![PHPUnit](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/actions/workflows/phpunit.yml/badge.svg)](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/actions/workflows/phpunit.yml)
[![Lint](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/actions/workflows/lint.yml/badge.svg)](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/actions/workflows/lint.yml)
[![Psalm](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/actions/workflows/psalm.yml/badge.svg)](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/actions/workflows/psalm.yml)
[![REUSE](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/actions/workflows/reuse.yml/badge.svg)](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/actions/workflows/reuse.yml)
[![Latest Release](https://img.shields.io/github/v/release/strobelpierre/nextcloud_oidc_claim_mapping?label=release)](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/releases/latest)
...
[![GitHub Stars](https://img.shields.io/github/stars/strobelpierre/nextcloud_oidc_claim_mapping)](https://github.com/strobelpierre/nextcloud_oidc_claim_mapping)

A Nextcloud app that maps OIDC token claims to **scalar Nextcloud user attributes** (display name, email, quota, country, language, etc.) via configurable rules. Hooks into the [user_oidc](https://github.com/nextcloud/user_oidc) app's `AttributeMappedEvent` so any rule can override the value computed natively by user_oidc.

> **⚠️ Status: V0.1.0 alpha** — this app is a conceptual fork of [oidc_groups_mapping](https://github.com/strobelpierre/nextcloud_oidc_groups_mapping) (v1.1.0). The two apps share the rule engine, claim resolver, frontend skeleton, and Mustache template support. The split was made because Nextcloud's `AttributeMappedEvent` covers scalar user attributes while groups have their own mapping path inside user_oidc. Keeping the two surfaces in separate apps avoids overloading either UI.
>
> **Roadmap**
> - **V1 (this app, in progress)** — scalar user attributes only (no groups). Rules target `displayName`, `email`, `quota`, `country`, etc., from the `TargetRegistry` mirror of user_oidc 7.4.
...
Configure rules through the Nextcloud admin panel under **Administration &rarr; OIDC Claim Mapping**.
...
- `GET /ocs/v2.php/apps/oidc_claim_mapping/api/v1/rules` — list all rules
- `PUT /ocs/v2.php/apps/oidc_claim_mapping/api/v1/rules` — replace all rules
- `POST /ocs/v2.php/apps/oidc_claim_mapping/api/v1/rules/simulate` — simulate rules against a token
...
Install directly from the [Nextcloud App Store](https://apps.nextcloud.com/apps/oidc_claim_mapping):

- **Via admin UI:** Administration → Apps → search "OIDC Claim Mapping" → Install
- **Via OCC:** `php occ app:install oidc_claim_mapping`

### From release tarball

```bash
cd /var/www/html/custom_apps/
wget https://github.com/strobelpierre/nextcloud_oidc_claim_mapping/releases/latest/download/oidc_claim_mapping.tar.gz
tar xzf oidc_claim_mapping.tar.gz
php occ app:enable oidc_claim_mapping
```
...
```bash
cd /var/www/html/custom_apps/
git clone https://github.com/strobelpierre/nextcloud_oidc_claim_mapping.git oidc_claim_mapping
cd oidc_claim_mapping
...
```bash
make appstore
# Output: build/oidc_claim_mapping.tar.gz
```
