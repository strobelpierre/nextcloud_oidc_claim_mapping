...
export default {
	...
	data() {
		const rulesJson = loadState('oidc_claim_mapping', 'rules', '{}')
		const parsed = typeof rulesJson === 'string' ? JSON.parse(rulesJson) : rulesJson

		return {
			tab: 'visual',
			mode: parsed.mode || loadState('oidc_claim_mapping', 'mode', 'additive'),
			rules: parsed.rules || [],
			dirty: false,
			saving: false,
			editingIndex: null,
	...
	methods: {
		...
		async onSave() {
			...
			try {
				const url = generateOcsUrl('/apps/oidc_claim_mapping/api/v1/rules')
